...
public class GenerateMaze : MonoBehaviour
{
    ...
    public int currCollectibles = 0;
    public List<MazePiece> longestPath;
    void Start()
    {
        int currLongestPathLength = 0; //dont need this probably
        var currLongestPath = new List<MazePiece>();
        
        InstantiateMazePieces();
        Generate(null, _maze[0, 0]);
        SetMazePieceAttributes();
        GetLongestPath(_maze[0, 0], currLongestPathLength, currLongestPath);
        GenerateStartAndEndPoint();
        GenerateCollectibles();
        GenerateObstacles();
        //PrintPathList(longestPath);
    ...
    private void GenerateObstacles()
    {
        ...
        for(int i = 0; i < spawnObstacleTries; i++) //try to spawn obstacle a set number of times, can have a bool that states if generation succeeded or not?
        {
    ...
    private void PrintConnectedPieces(MazePiece mazePiece)
    {
    ...
    private void PrintPathList(List<MazePiece> pathList)
    {
    ...
    private void GetLongestPath(MazePiece mazePiece, int currentLongestPath, List<MazePiece> currentLongestPathList)//need logic for when we find a longer path on the maze
    {
        currentLongestPath++; //dont need this probably
        currentLongestPathList.Add(mazePiece);
        //Debug.Log(mazePiece.transform.position.ToString());
        //Debug.Log(currentLongestPath);
        //Debug.Log(currentLongestPathList.Count);

        //we are at an endpoint in the maze
        if(mazePiece.connectedPieces.Count == 0)
        {
            //Debug.Log("dead end");
            ...
            RemovePiecesUntilFork(currentLongestPathList); //does not work if we have a fork that leads into another fork down the line.
        }
        else
        {
            foreach(var connected in mazePiece.connectedPieces)
            {
                GetLongestPath(connected, currentLongestPath, currentLongestPathList);
            }
        }
